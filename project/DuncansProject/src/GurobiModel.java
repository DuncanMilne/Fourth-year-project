...
public class GurobiModel extends Algorithm {
	...
	private void assign3aConstraints(Algorithm a) throws GRBException {
		...
		for (Project p: a.projects) {
			underCapacityProjects.add(grbmodel.addVar(0.0, 1.0, 0.0, GRB.BINARY, p.name + " under capacity"));
		}
		...
		for (Student s:assignedStudents){
			...
			for (Project p:s.preferenceList){
				...
				// have to make sum of equal to 0 or 1?
				...
				for (int j = p.lecturer.projects.indexOf(p)+1; j < p.lecturer.projects.size(); j ++) {
					Dijk.addTerm(1.0, s.grbvars.get(j));
				}
				...
				grbmodel.addConstr(Cijk, GRB.LESS_EQUAL, 1.0, "name");
				grbmodel.addConstr(Dijk, GRB.GREATER_EQUAL, 0.0, "name");
    	 ...
    	 for (Student i1:a.assignedStudents) {
    		 for (Student i2:a.assignedStudents) { // can find vi' by getting grbvar at certain indexes
    			 if (i1 != i2){
	    			 ...
	    			 //System.out.println(rhs.getConstant());
	    			 //System.out.println(rhs.getCoeff(0));  don't understand how this works. need to work out how this works and if its doing the right thing.
